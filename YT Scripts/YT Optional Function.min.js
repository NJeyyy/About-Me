...
// @icon           https://i.ibb.co/1r3t1wT/YTOptional-Function-Min-Userscript-Icon.png
// @grant          GM.setValue
// @grant          GM.getValue
// @noframes
// ==/UserScript==
...
async function start_YTOptionalFunction() {
  if(ISE("div#LutleLoadingSpinIcon")) console.warn("BLOCKED TO LAUNCH MORE THAN ONCE.");
  else {
    var e;
    if(window.location.href.match(/https\:\/\/www\.youtube\.com\/watch*/) && (e = setInterval(BLOCKAUTOPLAY, 100)), console.log('Interval ID: '+e), await waitFor((e => "complete" == document.readyState)), await waitFor((e => "visible" == document.visibilityState || "visible" == document.webkitVisibilityState)), SidebarPARENT(), await LittleLoadingIcon(), window.location.href.match(/https\:\/\/www\.youtube\.com\/watch*/)) {
    ...
    for(await Theme_BasedTime(), await sleep(70), ToggleTHEMEBTN(), await Startonlyonvideopage(), await sleep(70), await waitFor((e => "visible" == document.visibilityState || "visible" == document.webkitVisibilityState)), await sleep(90), BLOCKAUTOPLAY(), document.addEventListener("click", ClearYTSearchBox_Byfewact); ISE(".LoadingOverlayELEMENTS");) ISE(".LoadingOverlayELEMENTS").remove();
    for(; document.getElementById("LutleLoadingSpinIcon");) document.getElementById("LutleLoadingSpinIcon").remove();
    window.location.href.match(/https\:\/\/www\.youtube\.com\/watch*/) && (clearInterval(e), ISE("ytd-app").focus()), console.log("YT Optional Function");
  }
}
async function Startonlyonvideopage() {
  var e = window.location.href;
  "true" == sessionStorage.getItem("AlreadyRUN_YTOptionalFunction1") && (document.removeEventListener("keyup", YTAds_Shortcut, !1), await sleep(90), sessionStorage.setItem("AlreadyRUN_YTOptionalFunction1", !1)), e.match(/https\:\/\/www\.youtube\.com\/watch*/) ? (await waitFor((e => ISE("div#info-strings yt-formatted-string.ytd-video-primary-info-renderer"))), await document.addEventListener("keyup", YTAds_Shortcut, !1), await document.addEventListener("keypress", EnterCommentKEYFunc), ISE(".view-count.ytd-video-view-count-renderer").addEventListener("contextmenu", (function () {
  ...
  }), {once: !0}), ISE(".view-count.ytd-video-view-count-renderer").setAttribute("title", ISE("div#info-strings yt-formatted-string.ytd-video-primary-info-renderer").textContent), document.location.href.match(/&list=/) && (ISE("video").onplay = e => {
    var t = e.path[0], o = t.duration - 0.15;
    t.currentTime >= o && (t.pause(), t.currentTime = o, t.onplay = n => {
      t.currentTime >= o && (t.pause(), t.currentTime = o, console.error(e.path[0].currentTime), console.warn("This video was about to end... Moreover, it seems that your video was shown in a playlist.\nSo... it's best to stop here to not proceed to the next video without your consent."));
    });
  }, console.log("This video is played in a playlist.. correct?")), sessionStorage.setItem("AlreadyRUN_YTOptionalFunction1", !0), console.log("DONE! that's script that's only run in video page!")) : console.log("This script is not working here =w="), await sleep(50);
}
...
async function Theme_BasedTime() {
  ...
  var theme_SELECTIONONE = await GM.getValue("theme_SELECTION1", "Dark");
  var theme_SELECTIONTWO = await GM.getValue("theme_SELECTION2", "Light");
  var CurrentTheme;
  if (timeOfDay >= time_NIGHT1 || timeOfDay <= time_NIGHT2) {
    var CurrentTheme = theme_SELECTIONONE;
    console.log(theme_SELECTIONONE, "Theme");
  } else if (timeOfDay >= time_DAY1 && timeOfDay <= time_DAY2) {
...
sessionStorage.setItem("AlreadyRUN_YTOptionalFunction1", !1);
GM.setValue("SubmitCommentEvent_ErrorCounter", 0), start_YTOptionalFunction();
window.addEventListener("locationchange", async function() {
  if(!ISE("div#LutleLoadingSpinIcon")) {
    await waitFor((e => "visible" == document.visibilityState || "visible" == document.webkitVisibilityState)), await LittleLoadingIcon();
    var e = setInterval(BLOCKAUTOPLAY, 100);
		console.log('Interval ID: '+e);
    if(await sleep(150), window.location.href.match(/https\:\/\/www\.youtube\.com\/watch*/)) {
      let e = document.createElement("style");
    ...
    for(await Theme_BasedTime(), await Startonlyonvideopage(), await waitFor((e => "visible" == document.visibilityState || "visible" == document.webkitVisibilityState)); ISE(".LoadingOverlayELEMENTS");) ISE(".LoadingOverlayELEMENTS").remove();
    for(; document.getElementById("LutleLoadingSpinIcon");) document.getElementById("LutleLoadingSpinIcon").remove();
    return window.location.href.match(/https\:\/\/www\.youtube\.com\/watch*/) && (clearInterval(e), ISE("ytd-app").focus()), void console.log("YT Optional Function");
  }
