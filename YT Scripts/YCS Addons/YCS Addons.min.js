...
function doc_keyUp(a) {
  ...
  } else if((Hotkey2 && Hotkey1 && a.shiftKey && !(a.MetaKey | a.CtrlKey)) && (ISE(".ycs-app").classList.contains("YCS-ModCSSByNJ1n9_Overlay") || ISE(".ycs-app").classList.contains("YCS-ModCSSByNJ1n9_OverlaySimple"))) {
    ISE("#CollapseButton").click();
  }
}
async function AddmoreButtonFunctionality() {
  ...
  if(SearchAddons) {
    ...
    if(timestampMATCH) {
      ...
      d.innerHTML += timestampMATCH_compiledData.style + timestampMATCH_compiledData.LoadingBarCSS + timestampMATCH_compiledData.ContainerResultCSS;
  ...
  if(!document.querySelector('#ycs-search-result #ycs_wrap_comments div.ycs-render-comment:not(.ycs-show_more_block, .Pin-Comments) .Add-pinnedComments')) {
    document.querySelectorAll('#ycs-search-result #ycs_wrap_comments div.ycs-render-comment:not(.ycs-show_more_block, .Pin-Comments)').forEach(elementS => {
      ...
      PinBTN.addEventListener('click', function () {
        ...
        setTimeout(_ => {
          elementS.querySelector('.Add-pinnedComments').removeAttribute('style');
        }, 1000);
        var b = elementS.cloneNode(true);
        ...
        DumpBTN.addEventListener('click', async function (e) {
          var a = e.path.find(Elm => Elm.classList.contains('Pin-Comments'));
          if(a) {
            a.style.setProperty('transform', 'scaley(0)');
            await sleep(610);
            a.remove();
            document.getElementById('YCBookmarkBTN').setAttribute('title', 'Comment pinned: ' + document.getElementById('YCBookmarkElm').children.length);
          }
        });
        b.appendChild(DumpBTN);
        b.classList.add("Pin-Comments");
        b.style.setProperty('display', 'inline-block;');
        while(b.querySelector('.ycs-open-comment')) {
          b.querySelector('.ycs-open-comment').remove();
        }
        while(b.querySelector('.ycs-open-reply')) {
          b.querySelector('.ycs-open-reply').remove();
        }
        document.getElementById('YCBookmarkElm').appendChild(b);
        while(document.querySelector('#YCBookmarkElm .Add-pinnedComments')) {
          document.querySelector('#YCBookmarkElm .Add-pinnedComments').remove();
        }
        for(var i = 0; i < b.querySelectorAll('a.ycs-cpointer[data-offsetvideo]').length; i++) {
          b.querySelectorAll('a.ycs-cpointer[data-offsetvideo]')[i].addEventListener('click', async function (a) {
            a.preventDefault();
            if(a.target.getAttribute('data-offsetvideo')) {
              console.log(a.target.getAttribute('data-offsetvideo'));
              document.querySelector("#movie_player").seekTo(a.target.getAttribute('data-offsetvideo'), true);
            }
            return false;
          });
        }
...
async function WhatTextIsSearchedin_YCS() {
  ...
  d.setAttribute("class", "YCSADDONSElem");
