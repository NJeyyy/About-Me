		...
		if (SearchInput) {
			...
			if (!SearchInput || SearchInput.length == 0) {
				...
				if (SE("#ycs-search-result #ycs_wrap_comments div.ycs-render-comment:not([SpamCommentDisplay])")) {
					...
					var i0 = 0;
					var ExtractComment1 = setInterval(async function() {
						if (i0 < SE("#ycs-search-result #ycs_wrap_comments div.ycs-render-comment:not([SpamCommentDisplay])").length) {
							CurrentProgress++;
							var iN = i0 + 1;
							ISE('#YCS_TimestampMatchResult .LoadingBar').style.setProperty('width', (CurrentProgress / TotalProgress) * 100 + '%', 'important');
							if (SE("#ycs-search-result #ycs_wrap_comments div.ycs-render-comment")[i0].querySelector(".ycs-comment__main-text").textContent.match(regexTOyk)) {
								if (SE("#ycs-search-result #ycs_wrap_comments div.ycs-render-comment")[i0].querySelector("a.ycs-gotochat-video[href]")) {
									if (SE("#ycs-search-result #ycs_wrap_comments div.ycs-render-comment")[i0].querySelectorAll("a.ycs-gotochat-video[href]").length > 1) {
										var i1 = 0;
										var ExtractComment2 = setInterval(function(){
											if (i1 < SE("#ycs-search-result #ycs_wrap_comments div.ycs-render-comment")[i0].querySelectorAll("a.ycs-gotochat-video[href]").length) {
										...
										await sleep(SE("#ycs-search-result #ycs_wrap_comments div.ycs-render-comment")[i0].querySelectorAll("a.ycs-gotochat-video[href]").length*0.000001);
							...
							clearInterval(ExtractComment1);
							return;
						}
					}, 0.001);
					await sleep(SE("#ycs-search-result #ycs_wrap_comments div.ycs-render-comment:not([SpamCommentDisplay])").length*0.001);
					//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
					//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
					console.groupEnd();
					...
					if (a__data && a__data["Matched_Comment"].length != 0) {
						...
						if (a__data["TextMatch"].length != 0) {
							for (var iElemB_1 = 0; iElemB_1 < a__data["TextMatch"].length; iElemB_1++) {
								if (!a__data["TextMatch"][iElemB_1].classList.contains("Textmatch_yttimestampEXP")) {
									a__data["TextMatch"][iElemB_1].classList.add("Textmatch_yttimestampEXP");
								}
							}
							var TextMatchElementsONLY = Object.values(SE('.Textmatch_yttimestampEXP'));
							for (let iElemB_2 = 0; iElemB_2 < TextMatchElementsONLY.length; iElemB_2++) {
								ISE("#ycs_wrap_comments").prepend(TextMatchElementsONLY.slice().reverse()[iElemB_2]);
							}
						}
					}
					console.groupEnd();
